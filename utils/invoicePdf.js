...
// Generate a professional invoice PDF and return a Buffer
async function generateInvoicePdfBuffer(invoice, userEmail) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const fmt = (n, currency) => `${currency} ${new Intl.NumberFormat('en-IN', { maximumFractionDigits: 0 }).format(Number(n || 0))}`;
      ...
      doc.fillColor('#374151').fontSize(11).text(`${userEmail}`, 48, 140);
      ...
      doc.text(fmt(invoice.amount, invoice.currency), 400, y, { width: 70, align: 'right' });
      doc.text(fmt(invoice.amount, invoice.currency), 480, y, { width: 67, align: 'right' });
      y += 24;
      ...
      doc.fillColor('#111827').fontSize(11).text(fmt(invoice.amount, invoice.currency), 480, y, { width: 67, align: 'right' });
      y += 16;
      doc.fillColor('#6b7280').fontSize(10).text('Tax (0%)', 400, y, { width: 80, align: 'right' });
      doc.fillColor('#111827').fontSize(11).text(fmt(0, invoice.currency), 480, y, { width: 67, align: 'right' });
      y += 16;
      drawDivider(y + 6);
      y += 18;
      doc.fillColor('#111827').fontSize(12).text('Total', 400, y, { width: 80, align: 'right' });
      doc.fontSize(12).text(fmt(invoice.amount, invoice.currency), 480, y, { width: 67, align: 'right' });
      y += 30;
      ...
      doc.fillColor('#6b7280').fontSize(9)
        ...
        .text('Need help? Contact audix@gmail.com', 48, y + 14)
        .text(`© ${new Date().getFullYear()} Audix. All rights reserved.`, 48, y + 28);
